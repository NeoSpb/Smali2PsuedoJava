...
public class Utils {
    ...
    public static ArrayList<String> processParamTypes(String line) {
        ...
        for (int i = 0; i < line.length(); i++) {
            ...
            if (curElem.length() == 0) {
                ...
                } else curElem.append(curChar);
            } else {
                if (curChar.equals(";")) {
                    params.add(arr.toString() + curElem);
        ...
        if (type.equals("V")) return "void";
            ...
            if (type.startsWith("L")) {
                ...
                if (type.substring(type.length() - 1).equals(";"))
                    return type.substring(1, type.length() - 1) + arrAppend;
    ...
    public static void write(BufferedWriter writer, String line, int numTabs) throws IOException {
        ...
        for (int i = 0; i < numTabs; i++) output.insert(0, "\t");
    ...
    public static String padToWide(String input, int direction) {
        ...
        if (direction == Utils.PADDING_INFRONT) {
            ...
            while (inputBuilder.length() < 16) inputBuilder.insert(0, "0");
            input = inputBuilder.toString();
        } else if (direction == Utils.PADDING_BEHIND) {
            // Pad Behind
            StringBuilder inputBuilder = new StringBuilder(input);
            while (inputBuilder.length() < 16) inputBuilder.append("0");
            input = inputBuilder.toString();
    ...
    public static String padToNormal(String input, int direction) {
        ...
        if (direction == Utils.PADDING_INFRONT) {
            // Pad Infront
            StringBuilder inputBuilder = new StringBuilder(input);
            while (inputBuilder.length() < 8) inputBuilder.insert(0, "0");
            input = inputBuilder.toString();
        } else if (direction == Utils.PADDING_BEHIND) {
            // Pad Behind
            StringBuilder inputBuilder = new StringBuilder(input);
            while (inputBuilder.length() < 8) inputBuilder.append("0");
            input = inputBuilder.toString();
